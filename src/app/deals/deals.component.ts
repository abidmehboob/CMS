...
export class DealsComponent implements OnInit {
  ...
  constructor(
    ...
    this.subscriptions[4] = this.subcategoryService.subcategoryData$.subscribe(
      (data) => {
        console.log(data)
        if (data) {
          this.privateHired = data['content'];
    ...
    this.subscriptions[0] = this.listingService.listingData$.subscribe(
      ...
      this.subscriptions[1] = this.categoryService.categoryData$.subscribe(
        (data) => {
          if (data) {
            this.sightseens = data['content'];
          }
        });
  
      
    this.subscriptions[2] = this.categoryService.dealsData$.subscribe(
      (data) => {
        if (data) {
  ...
  updateFilter(event) {
    const val = event.target.value.toLowerCase();
      if(val.length === 0){
         ...
         setTimeout(() => {
          this.categoryService.getAllDeals(0,val);
  ...
  onSubmit() {
    ...
    if (this.name === null) {
      ...
      this.notifyService.showWarning('Deal Type required', 'Error');
      this.showHighlight('dealType');
      return false;
    ...
    let object = {
      ...
      "private_hired_id" : this.privateH
    }
    if(this.mode == 1){
